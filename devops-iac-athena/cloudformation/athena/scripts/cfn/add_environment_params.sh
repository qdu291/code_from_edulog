...
$AWS_CMD ssm put-parameter --overwrite --cli-input-json "{\"Name\":\"/edulog/athena/$env_type/$tenant/be_asg\",\"Value\":\"$be_asg\",\"Type\":\"String\"}"
$AWS_CMD ssm put-parameter --overwrite --cli-input-json "{\"Name\":\"/edulog/athena/$env_type/$tenant/cq_asg\",\"Value\":\"$cq_asg\",\"Type\":\"String\"}"
...
$AWS_CMD ssm put-parameter --overwrite --cli-input-json "{\"Name\":\"/edulog/athena/$env_type/$tenant/wosnos_id\",\"Value\":\"$wosnos_id\",\"Type\":\"String\"}"

# Create CloudWatch Agent Configuration in SSM Parameter Store
sed -i "s/ENV_NAME/${tenant}/g" cloudwatch.conf
$AWS_CMD ssm put-parameter --overwrite --value "$(cat cloudwatch.conf)" --type String --name AmazonCloudWatch-athena-${env_type}-${tenant}