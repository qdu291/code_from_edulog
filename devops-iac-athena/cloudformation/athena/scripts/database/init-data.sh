...
mkdir -p /tmp/base_db
cd /tmp/base_db
aws s3 cp --recursive $s3_path .

## IMPORT DATA
psql -U postgres -h $db_host -d "Athena" -f athena.base.sql
psql -U postgres -h $db_host -d "Athena" -f geocode.settings.sql
psql -U postgres -h $db_host -d "Athena" -f ivin.base.sql
psql -U postgres -h $db_host -d "Athena" -f user_profile.clean.sql
psql -U postgres -h $db_host -d "Athena" -f user_profile.base.sql
